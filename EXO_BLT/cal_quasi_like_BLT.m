function [qlike,tres] = cal_quasi_like_BLT(param,y,n,sf,index,Pbullet,restrict);
% calculates the quasi likelihood for theta structure theta in the regional model with the BLT exogeneity condition.
%
% The structure is characterized by the index = [c(i),c(i)^*] and the vector sf of dimensions. 
% Here sf(i) denotes the dimension of the regional variables, 
%      c(i) the number of common trends in the regional model i
%      c(i)^* the rank of K_{i,c}^* indicating the common trends spilling
%      over. 
%
% SYNTAX: [qlike,tres] = cal_quasi_like_BLT(param,y,n,sf,index,Pbullet,restrict);
%
% INPUTS:  param ... d x 1 vector of parameter values (fed into param2syst,
%                       see there for description)
%          y     ... T x (sist + si) matrix of observations (first star,
%                       then regional)
%          n     ... integer; state dimension
%          sf    ... pair of integers, [si,sist].
%          index ... pair of integers; [ci,cist] 
...
sist = sf(2);
if (stot ~= (si+sist))
    warning('Dimensions do not fit, adjusting sist!')
    sist = stot - si; 
end
ci = index(1);
cist = index(2); 

% convert params into system
[th] = param2th_BLT(param,n,sf,index); 
A= th.A;
K= th.K;
C= th.C;
B = th.B;
...
if Pbullet == 0 
    xh = ltitr(A-K*C,Ktil,y,zeros(n,1));
    tres = yi-xh*C'-ystar*th.D';
...
%     tres = reshape(ttv2,s,T)';
    Omegah = tres'*tres/T;
    qlike = qlike+ T*log(det(Omegah)) + T*sf(1); %trace(inv(Omega)*Omegah);
    return;
end


if ci>0 % there is an integration going on! 
    % take out the exo vars
    xh = ltitr(A-K*C,Ktil(:,1:sist),ystar,zeros(n,1));
    ty = yi-xh*C'-ystar*th.D';
    % find tilde k. via C1: 
    C1 = C(:,1:ci);
    Pi = C1*inv(C1'*C1)*C1';
    Cbull = [C(:,ci+1:end)];
    ...
    %ty = y(:,(sist+1):end); 
    ty(2:end,:)=ty(2:end,:)-ty(1:end-1,:)*Pi;
...
if Pbullet>0
    ...
    x0= zeros(n,1);
    tres = ty*0;
    tres(1,:)=ty(1,:)-x0'*tilC'; %-ystar(1,:)*tilD';
    Omegat= tilC*P0*tilC'+Omega;
    Kt = (tilA*P0*tilC'+tilK*Omega)*inv(Omegat);
    xf = tilA*x0+ Kt*tres(1,:)'; % + tilB*ystar(1,:)';
    ...
    for t=2:T % filter equations
        ...
        iOm = inv(Omegat);
        tres(t,:)= ty(t,:) - xf'*tilC'; %-ystar(t,:)*tilD';
        Kt = (tilA*Pkg1*tilC'+tilK*Omega)*iOm;
        xf = tilA*xf+ Kt*tres(t,:)';% + tilB*ystar(t,:)';
    ...
    Ts = size(ty,1);
    tres = ty-xh*C'; %-ystar*th.D';
    %xt = ltitr(Abar,[tilK,tilB],[ty(1:Ts-1,:),ystar(1:Ts-1,:)],x0);
    xt = ltitr(Abar,[tilK],[ty(1:Ts-1,:)],x0);
    tres = ty(1:Ts,:)-xt*tilC'; %-ystar(1:Ts,:)*tilD';
    %[tres,~] = est_initial_val(tres,Abar,tilK,tilC);
    
    Omegat =  tres'*tres/(Ts);
    % update likelihood
    qlike = Ts*log(det(Omegat)) + sf(1)*Ts;
end
