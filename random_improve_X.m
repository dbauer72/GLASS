function result = random_improve_X(result,k,M);
...
switch ur
    ...
    otherwise
        ll_cur = cal_quasi_like_X(param,z,s,m,n,0,D0,Pbull,rest);
end
...
for j=1:M
    ...
    switch ur 
        ...
        otherwise
            [parn,exitflag] = fminunc(@(x) cal_quasi_like_X(x,z,s,m,n,0,D0,Pbull,rest),param+randn(length(param),1)*k,options);
            ll_n = cal_quasi_like_X(parn,z,s,m,n,0,D0,Pbull,rest);
    end
    end
    if ll_n< ll_cur
        param = parn;
...
if norm(param-param_old)>10^(-10) % changes have been achieved
    ...
    switch ur
        ...
        otherwise
            [th,par]  = param2th_X(param(n_par_om+1:end),s,m,n,0,D0,rest);
            [llc,resc] =  cal_quasi_like_X(param,z,s,m,n,0,D0,Pbull,rest);
    end
